...
class RenderPipeline {
  ...
  Future<File> exportStory({
    ...
    final workplace = await Directory('${tempDir.path}/story_${DateTime.now().millisecondsSinceEpoch}')
    ...
    try {
      ...
      if (state.isVideo) {
        ...
        backgroundFile = await exporter.export(state, outputDirectory: workplace);
      } else {
      ...
      final serializedJson = SerializationService.serializeState(layers: state.layers);
      ...
      final zipFile = File('${tempDir.path}/story_${DateTime.now().millisecondsSinceEpoch}.zip');
